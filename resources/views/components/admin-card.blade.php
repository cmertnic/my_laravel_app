<div class="flex justify-center items-center bg-gray-100">
    <div class="bg-white shadow-md rounded-lg p-4 max-w-md w-full mb-2">
        ...
        <p><strong>Клиент:</strong> {{ $report->client->name ?? 'Неизвестный клиент' }}</p> <!-- Добавляем информацию о клиенте -->
        <p><strong>Адрес:</strong> {{ $report->address }}</p>
        <p><strong>Контакты для связи:</strong> {{ $report->contact }}</p>
        <p><strong>Дата:</strong> {{ $report->date }}</p>
        <p><strong>Время:</strong> {{ $report->time }}</p>
        <p><strong>Услуга:</strong> {{ $report->service->title }}</p>
        <p><strong>Тип оплаты:</strong> {{ ucfirst($report->payment) }}</p>
